...
class HashTableProbing:
    ...
    def put(self, key, value):
        ...
        if self.count >= self.size:
            raise Exception("Hash table is full")  # Table is full
        idx = self._hash(key)
        # Linear probing: move to the next slot until an empty slot or same key is found
        while self.keys[idx] is not None and self.keys[idx] != key:
            idx = (idx + 1) % self.size  # Circularly probe the next slot
        if self.keys[idx] is None:
            self.count += 1  # Inserting a new key
        self.keys[idx] = key
    ...
    def get(self, key):
        ...
        """
        idx = self._hash(key)
        start = idx  # Record starting position (for wrap-around detection)
        while self.keys[idx] is not None:
            if self.keys[idx] == key:
                return self.values[idx]  # Key found
            idx = (idx + 1) % self.size  # Move to next slot
            if idx == start:
