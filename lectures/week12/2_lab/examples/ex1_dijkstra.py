...
def dijkstra(graph, start):
    ...
    while pq:
        d, u = heapq.heappop(pq)  # Pop the node with the current minimum distance

        # Skip nodes already processed with a shorter path (lazy deletion)
        if d > dist[u]:
            continue

        # Perform relaxation on adjacent nodes
        for v, w in graph[u]:
            if dist[u] + w < dist[v]:  # If a shorter path is found
                dist[v] = dist[u] + w  # Update the distance
                prev[v] = u            # Record previous node (for path reconstruction)
                heapq.heappush(pq, (dist[v], v))  # Push onto the heap
